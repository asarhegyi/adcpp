function [L,grad] = Likelihood_wrt_ABC(x,Tl,sigma,meas_data,runmod)
...
if ~isempty(Y_index_others)
    prob(Y_index_others) = .5*(erf( lambda1*( Tl(Y_index(Y_index_others)+1) - input_signal(Y_index_others))/sigma/sqrt(2)) ...
        - erf( lambda1*( Tl(Y_index(Y_index_others)) - input_signal(Y_index_others))/sigma/sqrt(2)));
end%if

if ~isempty(Y_index_0)
    prob(Y_index_0) = 0.5*(1+erf( lambda1*( Tl(1) - input_signal(Y_index_0))/sigma/sqrt(2)) );
end%if

if ~isempty(Y_index_N)
    prob(Y_index_N) = 0.5*(1-erf( lambda1*( Tl(N) - input_signal(Y_index_N))/sigma/sqrt(2)));
end%if
...
temp_0 = lambda1*(Tl(1)-input_signal(Y_index_0));
temp_N = lambda1*(Tl(N)-input_signal(Y_index_N));

temp_others_k = lambda1*(Tl(Y_index(Y_index_others)+1)-input_signal(Y_index_others));
temp_others_k_minus_1 = lambda1*(Tl(Y_index(Y_index_others))-input_signal(Y_index_others));
